...
namespace FourSquare.SharpSquare.Core
{
    public class SharpSquareAsync
    {
        ...
        public SharpSquareAsync(string clientId, string clientSecret)
        {
        ...
        public SharpSquareAsync(string clientId, string clientSecret, string accessToken)
        {
            this.clientId = clientId;
            this.clientSecret = clientSecret;
        ...
        private async Task<FourSquareSingleResponse<T>> GetSingle<T>(string endpoint, Dictionary<string, string> parameters, bool unauthenticated) where T : FourSquareEntity
        {
            ...
            var json = await Request(string.Format("{0}{1}?{2}{3}&v={4}", ApiUrl, endpoint, oauthToken, serializedParameters, ApiVersion), HttpMethod.Get);
        ...
        private async Task Post(string endpoint, Dictionary<string, string> parameters = null)
        {
            ...
            await Request(string.Format("{0}{1}?oauth_token={2}{3}", ApiUrl, endpoint, accessToken, serializedParameters), HttpMethod.Post);
        }
        ...
        {
            var user = await GetSingle<User>("/users/" + userId);
        ...
        */
        ...
        {
            var checkins = (await GetSingle<FourSquareEntityItems<Checkin>>("/users/" + userId + "/checkins", parameters)).response["checkins"];
        ...
        {
            var friends = (await GetSingle<FourSquareEntityItems<User>>("/users/" + userId + "/friends", parameters)).response["friends"];
        ...
        {
            var tips = (await GetSingle<FourSquareEntityItems<Tip>>("/users/" + userId + "/tips", parameters)).response["tips"];
            return tips.items;
        }
        ...
        {
            var venues = (await GetSingle<FourSquareEntityItems<VenueHistory>>("/users/self/venuehistory")).response["venues"];
            return venues.items;
        }
        ...
        */
        ...
        {
            var venueGroups = (await GetMultiple<FourSquareEntityItems<Venue>>("/venues/managed", parameters, true)).response["venues"];
        ...
        {
            var venues = (await GetSingle<FourSquareEntityItems<Venue>>("/venues/search", parameters, true)).response["groups"];
            return venues.items;
        }
        ...
        {
            var checkins = (await GetSingle<FourSquareEntityItems<Checkin>>("/venues/" + venueId + "/herenow", parameters, true)).response["hereNow"];
        ...
        {
            var tips = (await GetSingle<FourSquareEntityItems<Tip>>("/venues/" + venueId + "/tips", parameters, true)).response["tips"];
            return tips.items;
        }
        ...
        {
            var photos = (await GetSingle<FourSquareEntityItems<Photo>>("/venues/" + venueId + "/photos", parameters, true)).response["photos"];
            return photos.items;
        }
        ...
        {
            var links = (await GetSingle<FourSquareEntityItems<Link>>("/venues/" + venueId + "/links", true)).response["links"];
        ...
        public async Task SetVenueFlag(string venueId, string problem)
        {
            ...
            await Post("/venues/" + venueId + "/flag", parameters);
        }
        ...
        {
            var parameters = new Dictionary<string, string> {{"text", text}};
            await Post("/checkins/" + checkinId + "/addcomment", parameters);
        }
        ...
        public async Task DeleteChekinComment(string checkinId, string commentId)
        {
            ...
            await Post("/checkins/" + checkinId + "/deletecomment", parameters);
        }
        ...
        {
            await Post("/tips/" + tipId + "/markdone");
        }
        ...
        {
            var parameters = new Dictionary<string, string> {{"value", value}};
            await Post("/settings/" + settingId + "/set", parameters);
        }
        ...
        {
            var specials = (await GetSingle<FourSquareEntityItems<Special>>("/specials/search", parameters)).response["specials"];
